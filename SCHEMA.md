...
- `aggregate.scorecard` (batch, runs, loop) / `eval.json` (`eval`) —
  ...
  risk_free, stopped_when_done, loop_free, error_free, milestones_complete, milestones_in_order, milestones_reached, recovered_errors: {successes, runs, rate, ci95}}, spend {latency_s, cost_usd, tokens, steps, tool_calls: {n, mean, median, min, max, total}}, trajectory, tools, grounding, safety {flags, flag_kinds, flagged_runs, …}, milestones {runs, reached, total, complete_runs, out_of_order_runs, stalled_at {id: runs}, basis}, retrieval, time, risk_reward, judge?, graded_by}}, detection {measurable, reason?, modes[{mode, task, agent, graded_pass, signals[], caught, stalled_at}], caught, total, missed[], graded_pass, graded_pass_caught_otherwise, by_signal, controls {runs, flagged, false_positives[]}, basis, narrative}, per_run[], note}`; a run's `milestones {measurable, total, reached, progress, in_order, complete, stalled_at, last_reached_step, steps_after_last, reached_ids, missed_ids}` is how far it got before it stopped — at three hundred steps the difference between *failed* and *failed at the eleventh of thirteen checkpoints* is the whole reading — and a task whose golden entry names no milestones is left out of those counts rather than scored zero (`docs/EVAL.md`, `docs/HORIZON.md`). `detection.judge {measurable, reason?, judged, of, caught, missed, only_the_judge[{mode, task, agent}], also_caught_deterministically, controls_called_wrong, controls_called_wrong_runs[] (≤ 12, `controls_called_wrong_capped`), model, rubrics[], on_an_excerpt, basis, narrative}` scores a judging model the same way and keeps it **out of** `caught`, `missed` and `by_signal` — every other number on the card is computable from the traces alone, and a sampled verdict merged into them would end that silently. `only_the_judge` is the figure a judge is paid for: known failures every deterministic dimension passed and the model did not. A run the judge never read is neither caught nor missed (`judged` says how many it read), and a judge that calls the known-correct runs wrong is reported in the same breath. A trace's `outcome.judge` gains `rubric_name` (`strict`, `long-run` or `custom`; `harness.judge.RUBRICS`) and, when the steps were shown, `steps_shown`, `steps_total` and `steps_basis` — a long run does not fit in a prompt, so `--with-steps` sends the opening and closing with the gap named in the text (`... N steps omitted here (indexes a-b) ...`, `--steps-cap`) — or, with `--focus`, the steps the run itself flags (`deepcompare.excerpt.focus`: an error nothing repaired, a redundant stretch, an unverified write, a cycle, a policy breach, a beat the run's own rhythm skipped (`skipped_beat`: a gap holding two of a tool's periods, the period being the largest gap it repeats), an irreversible act with much of the run after it (`shipped_before_check`, `GATE_SHARE`), weighted by the stated `WEIGHTS` and clustered so one stall is one place to look, with a quarter of the budget held for each end), recorded as `steps_chosen_by` (`structure`|`position`|`all`, what happened rather than what was asked for) and counted on the card as `detection.judge.excerpts_chosen_by`. The selector reads the trace and the policy, including a task's own `forbidden_tools`/`forbidden_patterns` — constraints stated to the agent before the run — and never `milestones`, `expected` or `failure_mode` (`excerpt.effective_policy`). The verdict says how much of the run it is about. `scorecard()` takes an **iterable** of trajectories — nothing past `score_run` touches one, so a generator that loads a trace at a time scores a corpus larger than memory (4,000 runs of 260 steps is ~1 GB held at once, a few MB streamed). `panel.json` (`agentdiff panel`, harness) — `{kind: "panel", model, runs, asked, synthesis[{claim, affects[], cites[{cite {run, step, quote}, ok, why}]}], findings[{question, claim, affects, verification {citations[], cited, verified, supported, reason, basis}, supported, error, turns, tool_calls, tools_used, looked_before_answering, trace_id, raw}], supported, dropped_for_bad_citations, dropped[{claim, reason}], errors[], turns, tool_calls, memory_between_questions: false, basis}` — a judge reading a whole corpus through `corpus`, `runs`, `contrast`, `open`, `locate`, `read` and `flags`, asked for a synthesis rather than a grade. A finding reaches `synthesis` only when **every** citation verifies: the named run exists, the step index is in it, and the quoted fragment appears in that step's call, result or note (whitespace and case normalised). The check involves no model. `--checkpoint` writes after each finding and resumes. A run's `recovery` carries `unrecovered_at[]` — the step indexes of the errors the run never came back to, capped at `UNRECOVERED_CAP` (24) with `unrecovered_capped` saying when the cap bit: a count reads the same whether a run fell over at step 12 or step 212, and the page draws the difference. `detection.agent_judge` is the same block for an **Agent-as-a-Judge** run (`judge --agent`): the judge reads the trace through tools (`graph`, `locate`, `read`, and `flags` — the deterministic reading of `deepcompare.excerpt`) and gives one verdict per golden milestone, each in its own loop with no memory between them; `outcome.agent_judge {kind, model, requirements[{id, requirement, success, score, rationale, turns, tool_calls, tools_used, looked_before_answering, trace_id}], judged, of, met, success, score, failed_requirements[], turns, tool_calls, looked_at_all, self_judged (by model *family*), memory_between_requirements: false, basis, prior, agrees_with_prior, applied}`. Only a milestone's `id`/`label` reaches the judge, never its `evidence` (`agentjudge.requirements_of`). Both judge blocks carry `kappa` — `cohens_kappa`, agreement corrected for each side's base rate, unmeasurable with a reason when a verdict is constant, because raw agreement over a constant judge is not agreement. `detection` scores the evaluation rather than the agent: a golden task may say which failure it is *known* to contain (`failure_mode`, `failure_mode_agents`) or that it contains none (`known_correct`), and the block reports which known failures any dimension of the card caught, which it missed, how many were graded a pass anyway, and how many known-correct runs were flagged — unmeasurable, with the reason, when no golden task names one. A run's `trajectory` adds `redundant_stretch`, `redundant_steps` and `redundant_at[]` from `process.repeats {redundant_stretches [{from, to, steps}], longest_redundant_stretch, redundant_steps, redundant_basis}` — the longest contiguous run of steps repeating an earlier (call, observation) pair, at least `REDUNDANT_STRETCH` (3) long. It is a shape rather than a magnitude, so it needs no threshold fitted to a corpus: a run that re-derives work it already did produces one and a run that calls the same tool repeatedly to different effect does not. The agents' section sums it as `redundant_runs` and `redundant_steps`, and it is one of `detection`'s ten signals.
