...
| why a run took as long as it did | the Where the time went section | each second attributed to thinking, a named tool, or the answer; wasted steps hatched and named; a rationale with every number in its table; in the Panels view, two treemaps on one scale (area = seconds), the calls × time heat map, latency by tool and the tool matrix, in a grid you compose and the browser remembers |
