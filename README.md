...
| the story, as charts | the Story view | D3 charts in sequence: what happened (roles, answer-value arcs, the decisive ring, spend after the basis), the trace as a tree (task → runs → phases → steps → values, the fault's path in red, phases that fold), why (hypotheses at their scores with evidence), take forward (numbered pins per next action, what the fix buys, labelled an estimate) |
