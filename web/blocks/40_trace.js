  ...
  function model(ctx) {
    ...
    if (st.run && records && records[st.run] && Array.isArray(records[st.run].steps) && records[st.run].steps.length) {
      ...
      return {
        ...
        steps: tiled(steps), phases: [], marks: marks, tiled: TILE > 1 ? TILE : null,
    ...
    return {
      ...
      steps: tiled(steps2), phases: TILE > 1 ? [] : phasesOf(report, side, steps2), marks: marks2, tiled: TILE > 1 ? TILE : null,
      ...
      root.appendChild(H("p", { class: "trc-note", text:
        (st.x === "time"
          ? (folds.length ? plural(folds.length, "quiet stretch") + " folded (a gap over " + QUIET_S + "s, drawn at 6 + 6·log₂(1 + seconds))."
             : "No idle stretch to fold: every step of this run begins where the last one ended.")
