...
(function (global) {
  ...
  function weight(c) { return Math.pow(clamp01(c.impact), 0.7); }
   ...
   * mode a cluster is wMin + k·impact^0.7, a fold a fixed thin strip, an
   * open cluster a box wide enough for its marks; k is the largest value
   * that fits the wider band. In even mode every cluster is its seconds. */
  function scale(im, sides, states, avail, narrow, mode) {
    var wMin = narrow ? 4 : 9, gap = 2;
    ...
    sides.forEach(function (s) {
      var units = unitsOf(im[s], states, s);
      var n = units.length;
      var fixed = gap * Math.max(0, n - 1), sumW = 0, nClosed = 0;
      units.forEach(function (u) {
        if (u.type === "fold") fixed += wFold;
        else if (states.open[skey(s, u.id)]) fixed += openWidth(u.c, avail, narrow);
        else { sumW += weight(u.c); nClosed++; }
      });
      if (sumW > 0) k = Math.min(k, (avail - fixed - nClosed * wMin) / sumW);
      per[s] = { units: units, gap: gap, wFold: wFold };
    });
    if (!isFinite(k)) k = 0;
    k = Math.max(0, Math.min(k, avail * 0.5 - wMin));
    sides.forEach(function (s) {
      var p = per[s];
    ...
    ["a", "b"].forEach(function (side) {
      ...
      p.units.forEach(function (u, ui) {
        ...
        var rect = svg.append("rect").attr("class", "im-cluster" + (open ? " open" : "")).attr("data-id", c.id).attr("data-kind", c.kind || "work").attr("data-side", side).attr("data-lane", lane)
          .attr("x", x).attr("y", ry).attr("width", w).attr("height", rowH).attr("rx", 3)
        ...
        if (quiet) svg.append("rect").attr("x", x).attr("y", ry).attr("width", w).attr("height", rowH).attr("rx", 3).attr("fill", "url(#" + hid + ")").attr("pointer-events", "none");
        ...
        if (!open && w >= 30) {
          svg.append("text").attr("class", "im-clab").attr("x", x + 4).attr("y", ry + rowH / 2 + 4).attr("fill", op >= 0.55 ? "var(--bg)" : "var(--ink)")
            .text(fit(c.label || (c.kind === "quiet" ? "quiet" : c.kind || ""), w - 8));
        }
        if (open) {
