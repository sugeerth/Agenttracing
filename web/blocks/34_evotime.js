...
(function (global) {
  ...
  //: the fold law: 4–5 units ≈ 20px, 11 ≈ 27px, 100 ≈ 46px
  function foldW(units) { return 6 + 6 * Math.log(1 + Math.max(0, units)) / Math.LN2; }
  ...
  function constrict(wA, wB, quiet, unfolded, on, x0, x1) {
    ...
    segs.forEach(function (s) { if (s.fold) foldPx += foldW(s.b - s.a); else kept += s.b - s.a; });
    ...
    segs.forEach(function (s) {
      var w = s.fold ? foldW(s.b - s.a) * fs : (s.b - s.a) * k;
  ...
  AgentDiff.block({
    ...
    render: function (el, ctx) {
      ...
      function draw() {
        ...
        var sc = constrict(win[0], win[1], geo.quiet.list, unfolded, st.constrict, x0, x1);
        geo.sc = sc;
      ...
      function drawL2(body, sc, col, top) {
        ...
        grp.eps.forEach(function (ep, ri) {
          ...
          for (var i = 0; i < ep.n; i++) {
            var a = sc.x(ax.t0[i]), b = sc.x(ax.t0[i] + ax.dur[i]);
            if (b < x0 || a > x1) continue;
            var w = Math.max(2, b - a - 0.5), k = ep.kind[i], r = ep.rw[i], isErr = ep.fl[i].indexOf("e") >= 0;
            var sg = g.append("g").attr("class", "evt-step").attr("data-step", i).attr("data-kind", kindWord(k)).attr("data-flags", ep.fl[i]);
            if (k === 2) sg.append("rect").attr("x", a + (w - 9) / 2).attr("y", mid - 4.5).attr("width", 9).attr("height", 9).attr("fill", ep.success ? "var(--good)" : "var(--bad)");
