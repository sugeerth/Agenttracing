...
(function (global) {
  ...
  function entities(q, d) {
    ...
    function named(ids) {
      ...
      ids.forEach(function (id) {
        var a = " " + id.toLowerCase() + " ", b = " " + id.toLowerCase().replace(/_/g, " ") + " ";
        if (q.indexOf(a) >= 0 || q.indexOf(b) >= 0) { if (!best || id.length > best.length) best = id; }
      });
      return best;
    }
  ...
  intent("eval-rejected", {
    ...
    answer: function (q, E, d) {
      ...
      return { speaker: evalSpeaker(d), text: text, embed: "cov-flow", select: { family: "coevolution", value: sel }, sources: src, synthetic: true, followups: follow };
    },
  });
  ...
  intent("eval-keep", {
    ...
    answer: function (q, E, d) {
      ...
      return { speaker: evalSpeaker(d), text: text, embed: firstBlock(["evo-lineage", "cov-hindsight"]), select: pick && inCatalogue("evo-lineage") ? { family: "evolution", value: { gen: pick } } : null, sources: src, synthetic: true,
  ...
  intent("eval-metric", {
    ...
    answer: function (q, E, d) {
      ...
      return { speaker: evalSpeaker(d), text: text, embed: "cov-metric", select: { family: "coevolution", value: { metric: id } }, sources: src, synthetic: true,
  ...
  intent("eval-saw", {
    ...
    answer: function (q, E, d) {
      ...
      return { speaker: evalSpeaker(d), text: text, embed: "cov-flow", select: { family: "coevolution", value: { step: key, evalGen: null, candidate: null } }, sources: src, synthetic: true,
  ...
  intent("eval-compare", {
    ...
    answer: function (q, E, d) {
      ...
      return { speaker: me, text: text, more: more, embed: firstBlock(["evc-verdict", "evc-curves"]), select: null, sources: src, synthetic: true,
  ...
  intent("evo-changed", {
    ...
    answer: function (q, E, d) {
      ...
      return { speaker: PAGE, text: text, embed: "evo-step", select: { family: "evolution", value: { gen: s.to } }, sources: src, synthetic: d.synthetic,
  ...
  intent("evo-keep", {
    ...
    answer: function (q, E, d) {
      ...
      return { speaker: PAGE, text: text, embed: firstBlock(["evo-lineage", "evo-steps"]), select: { family: "evolution", value: { gen: r.id } }, sources: ["aggregate.evolution.recommended", "aggregate.evolution.best"], synthetic: d.synthetic,
  ...
  function route(q, d, ctx) {
    var nq = norm(q), E = entities(nq, d);
  ...
  function clearTurns() { family().reset(); }
  ...
  AgentDiff.block({
    ...
    render: function (el, ctx) {
      ...
      function setChips(list, evalFam) {
        chips.innerHTML = "";
        (list || []).filter(Boolean).forEach(function (q) {
          chips.appendChild(H("button", { type: "button", class: "chat-chip" + (evalFam && d.cov && /you|your|yourself|compare|keep|probes/.test(q) ? " eval" : ""), text: q, onclick: function () { ask(q); } }));
        });
      }
      function chipsFor(card) {
        var f = card && Array.isArray(card.followups) ? card.followups.filter(Boolean) : [];
        if (f.length < 3) defaultChips(d, ctx).forEach(function (c) { if (f.length < 5 && f.indexOf(c) < 0) f.push(c); });
        if (f.indexOf("what can you answer?") < 0) f.push("what can you answer?");
        return f;
      }
      function ask(q) {
        q = String(q || "").trim();
        ...
        var res = route(q, d, ctx);
        applySelection(res.card.select);
        var row = H("li", { class: "chat-turn new", "data-turn": String(i) });
        ...
        setChips(chipsFor(res.card), true);
        status.textContent = (res.card.cannot ? "No answer for that; " : "Answered by " + res.card.speaker + "; ") + "turn " + (i + 1) + " of " + st.turns.length + (st.turns.length >= MAX_TURNS ? " (the transcript keeps the latest " + MAX_TURNS + ")" : "") + ".";
      ...
      var cards = st.turns.map(function (turn) { return turn.q === null || turn.q === undefined ? welcome(d, ctx) : route(turn.q, d, ctx).card; });
      ...
      st.turns.forEach(function (turn, i) {
        var row = H("li", { class: "chat-turn", "data-turn": String(i) });
        if (turn.q) row.appendChild(H("p", { class: "chat-q", text: turn.q, "aria-label": "you asked: " + turn.q }));
        renderCard(H, cards[i], d, ctx, { ask: ask, live: !!liveSet[i], parent: row });
        log.appendChild(row);
      });
      setChips(chipsFor(cards[cards.length - 1]), true);
  ...
  AgentDiff.chat = {
    ...
    route: function (q) { var ctx = { aggregate: AgentDiff.state().data.aggregate || {}, report: null, task: AgentDiff.state().task }; try { ctx.report = (AgentDiff.state().data.reports || []).filter(function (r) { return r && r.task && r.task.id === ctx.task; })[0] || (AgentDiff.state().data.reports || [])[0] || null; } catch (err) { ctx.report = null; } var r = route(q, data(ctx), ctx); return { intent: r.intent, card: r.card }; },
