...
(function (global) {
  ...
  function ensureStyle() {
    ...
    var css = [
      ...
      "margin-top:7px;font-size:var(--fs-xs);color:var(--ink-3);line-height:1}",
      ...
      ".tj-read{margin-top:7px;min-height:15px;font-family:var(--mono);font-size:var(--fs-xs);",
      ...
      "border-radius:6px;padding:1px 7px;font-size:var(--fs-xs);line-height:1.5;cursor:pointer}",
      ".tj-nav button:hover:not([disabled]){border-color:var(--accent);color:var(--accent)}",
      ".tj-nav button[disabled]{opacity:.4;cursor:default}",
      ".tj-note{margin-top:6px;font-size:var(--fs-xs);color:var(--ink-2);border-left:2px solid var(--warn);padding-left:7px}",
      ".tj-seq{font-family:var(--mono);font-size:var(--fs-xs);color:var(--ink-2);display:flex;",
      "align-items:baseline;gap:6px;margin-top:2px}",
      ".tj-seq b{font-weight:600;font-size:var(--fs-xs);letter-spacing:.04em}",
      ...
      ".tj-ctl .grp button{border:0;background:var(--surface);color:var(--ink-2);font-size:var(--fs-xs);",
      ...
      ".tj-ctl select{font-family:inherit;font-size:var(--fs-xs);background:var(--surface);color:var(--ink-2);",
      "border:1px solid var(--rule-2);border-radius:6px;padding:2px 4px}",
      ".tj-ctl select[disabled]{opacity:.45}",
      ".tj-status{font-family:var(--mono);font-size:var(--fs-xs);color:var(--ink-3)}",
      ".tj-trade{margin-top:8px;font-size:var(--fs-xs);color:var(--ink-2);border-left:2px solid var(--accent);",
      "padding-left:8px;line-height:1.45}",
      ".tj-trade b{display:block;font-size:var(--fs-xs);text-transform:uppercase;letter-spacing:.08em;",
  ...
  /* The walkthrough's fallback for driving the family cursor is a
   * CustomEvent — documented in 05_walkthrough.js as "any module may
   * listen". Until this listener existed, nothing did: when Tracks was
   * absent or its hit layout unrecognized, the event fired into silence
   * and the walkthrough degraded to a plain scroll. Now the event moves
   * the same cursor a hand click would. */
  try {
    document.addEventListener("agentdiff:select-step", function (event) {
      var d = event && event.detail;
      if (!d || typeof d.row !== "number" || d.row < 0) return;
      select(d.row, d.side === "a" || d.side === "b" ? d.side : null, null);
    });
  } catch (err) { /* no document: nothing to listen on */ }
  ...
  AgentDiff.block({
    ...
    relevance: function (ctx) {
      // the map carries the timeline as one of its two views; this block
      // is kept for the drawer (and for anyone who wants both at once)
      return 0;
    },
    render: function (el, ctx) { renderTracksInto(el, ctx, true); },
  });

  /* The timeline (columns / wall-clock time, with replay) drawn into any
   * host: its own block in the drawer, or the map's "timeline" view. */
  function renderTracksInto(el, ctx, withHeader) {
    bind(ctx);
    ensureStyle();
    ...
    if (withHeader) el.appendChild(sideHeader(report));
  ...
  function drawDetail(host, ctx) {
    ...
    host.appendChild(H("p", {
      class: "caveat",
      text: sel.pinned
        ? "Showing the step you selected in the map, the ribbon or a chip."
        : "Nothing selected yet, so this is " + (sel.why || "the first row") +
          ". Click any step in the map to change it.",
    }));
  }
  ...
  function ensureMapStyle() {
    ...
    var css = [
      ...
      "font-size:var(--fs-xs);color:var(--ink-3);align-items:center;line-height:1.2}",
      ".tjm-foot .k{display:inline-flex;align-items:center;gap:4px}",
      ".tjm-note{margin-top:6px;font-size:var(--fs-xs);color:var(--ink-2);",
      "border-left:2px solid var(--warn);padding-left:7px}",
      ".tjm-head{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:6px}",
      ".tjm-head .sp{flex:1}",
      ".tjm-view{display:inline-flex;border:1px solid var(--rule-2);border-radius:7px;overflow:hidden}",
      ".tjm-view button{border:0;background:var(--surface-2);color:var(--ink-2);font:600 var(--fs-s)/24px inherit;",
      "padding:0 10px;cursor:pointer}.tjm-view button+button{border-left:1px solid var(--rule)}",
      ".tjm-view button[aria-pressed=true]{background:var(--accent);color:#fff}",
      ".tjm-grid{display:grid;grid-template-columns:minmax(0,1fr) 380px;gap:14px;align-items:start}",
      "@media (max-width:1079px){.tjm-grid{grid-template-columns:minmax(0,1fr)}}",
      ".tjm-cell{min-width:0}",
      ".tj-inspector{min-width:0;border:1px solid var(--rule);border-radius:8px;background:var(--surface)}",
      ".tj-inspector .block-body{padding:10px 12px}",
      ".tj-inspector-title{font-size:var(--fs-xs);text-transform:uppercase;letter-spacing:.08em;font-weight:700;",
      "color:var(--ink-3);padding:8px 12px 0}",
      ".tjm-legend-more{margin-top:4px}.tjm-legend-more>summary{cursor:pointer;font-size:var(--fs-xs);",
      "color:var(--ink-3);list-style:none}.tjm-legend-more>summary::before{content:'▸ '}",
      ".tjm-legend-more[open]>summary::before{content:'▾ '}",
      ...
      ".tj-diff-body{font:var(--fs-s)/1.55 var(--mono);white-space:pre-wrap;word-break:break-word;",
      "color:var(--ink-2)}",
  ...
  // The selected claim edge, if any. Module state like Sel, and like Sel
  // it belongs to the task it was made on.
  var MapClaim = { task: null, index: null };

  function mapClaimSync(report, claimCount) {
    var task = report && report.task ? report.task.id : null;
    if (MapClaim.task !== task) { MapClaim.task = task; MapClaim.index = null; }
    if (MapClaim.index !== null
        && (MapClaim.index < 0 || MapClaim.index >= claimCount)) {
      MapClaim.index = null;
    }
  }
  ...
  AgentDiff.block({
    ...
    render: function (el, ctx) {
      ...
      // header row: the two runs, then the view switch (map | timeline)
      var head = H("div", { class: "tjm-head" }, [
        sideHeader(report),
        H("span", { class: "sp" }),
        H("div", { class: "grp tjm-view", role: "group", "aria-label": "map view" },
          [["map", "map"], ["timeline", "timeline"]].map(function (opt) {
            return H("button", {
              type: "button", text: opt[1], "data-mapview": opt[0],
              "aria-pressed": MapView.mode === opt[0] ? "true" : "false",
              onclick: function () {
                if (MapView.mode === opt[0]) return;
                MapView.mode = opt[0];
                try { ctx.signal("inspect"); } catch (e) { /* optional */ }
                el.innerHTML = "";
                AgentDiff.block ? null : null;
                render(el, ctx);
              },
            });
          })),
      ...
      // the grid: the drawing on the left, the step under the cursor on
      // the right — click a node and the step is read in the same view,
      // never three screens down. One column on narrow screens.
      var grid = H("div", { class: "tjm-grid" });
      var cell = H("div", { class: "tjm-cell" });
      ...
      if (MapView.mode === "timeline") {
        renderTracksInto(cell, ctx, false);
      } else {
        var host = sized(cell, ctx, function (box, avail) {
          drawMap(box, avail, ctx);
        });
        subscribe(cell, function () {
          host.innerHTML = "";
          drawMap(host, measure(cell), ctx);
        });
      }
      function paintInspector() {
        inspector.innerHTML = "";
        inspector.appendChild(H("div", { class: "tj-inspector-title", text: "Step under the cursor" }));
        var body = H("div", { class: "block-body" });
        inspector.appendChild(body);
        if (!rowsOf(report).length) { ctx.empty(body, "No alignment rows to open."); return; }
        drawDetail(body, ctx);
      }
      paintInspector();
      subscribe(inspector, paintInspector);
      function render(node, c) { BY_MAP.render(node, c); }
    },
  });
  var BY_MAP = { render: null };
  BY_MAP.render = function (el, ctx) {
    // re-entry for the view switch: the registered render, called again
    var entry = null;
    try { entry = AgentDiff._internals && AgentDiff._internals.blockEntry && AgentDiff._internals.blockEntry("trajectory-map"); } catch (e) { entry = null; }
    if (entry && entry.render) return entry.render(el, ctx);
    // fall back to a full page re-render
    if (AgentDiff._rerender) AgentDiff._rerender();
  };
  var MapView = { mode: "map" };
  ...
  function drawMap(host, avail, ctx) {
    ...
    var gutter = W >= 960 ? 240 : W >= 620 ? 200 : Math.max(84, Math.min(200, Math.round(W * 0.28)));
    ...
    // clickable (through a wider invisible twin, because a 1px dashed
    // curve is no click target): clicking selects the claim, rings both
    // endpoints, writes the claim into the readout line, and moves the
    // shared cursor to the carrying A step — the claim is never
    // tooltip-only information.
    mapClaimSync(report, claims.length);
    function pickClaim(k) {
      MapClaim.index = MapClaim.index === k ? null : k;
      if (MapClaim.index !== null) {
        var rowA = mapRowFor(report, "a", claims[k].a);
        if (rowA >= 0) { select(rowA, "a", ctx); return; }
      }
      notify();
    }
    claims.forEach(function (edge, k) {
      var pa = nodeXY("a", edge.a), pb = nodeXY("b", edge.b);
      if (!pa || !pb) return;
      var midX = (laneA + laneB) / 2;
      var stroke = edge.wrong ? C.bad : C.good;
      var picked = MapClaim.index === k;
      var d = "M" + (pa.x + 8) + " " + pa.y +
              " C" + midX + " " + pa.y + " " + midX + " " + pb.y +
              " " + (pb.x - 8) + " " + pb.y;
      var path = S("path", {
        d: d, fill: "none", stroke: stroke,
        "stroke-width": picked ? 2 : 1,
        "stroke-dasharray": picked ? null : "1.5,3",
        opacity: picked ? 1 : 0.75,
        class: "tjm-claim",
      });
      svgEl.appendChild(path);
      var hit = S("path", {
        d: d, fill: "none", stroke: "transparent", "stroke-width": 9,
        class: "tjm-claim-hit tj-hit", "data-claim": k,
      });
      hit.appendChild(S("title", {
    ...
    // the selected claim's endpoints, ringed on top of everything
    if (MapClaim.index !== null && claims[MapClaim.index]) {
      var edge = claims[MapClaim.index];
      [["a", edge.a], ["b", edge.b]].forEach(function (end) {
        var p = nodeXY(end[0], end[1]);
        if (!p) return;
        svgEl.appendChild(S("circle", {
          cx: p.x, cy: p.y, r: 13, fill: "none",
          stroke: edge.wrong ? C.bad : C.good,
    ...
    if (MapClaim.index !== null && claims[MapClaim.index]) {
      ...
      readout.appendChild(H("i", {
        text: claims.length + " claim edge(s) — click one, or the count "
              + "below, to read it",
      }));
    }
    if (claims.length || MapClaim.index !== null) host.appendChild(readout);

    var claimsChip = claims.length ? H("button", {
      class: "k", type: "button",
      style: { border: "1px solid var(--rule-2)", borderRadius: "5px",
               background: "var(--surface)", color: "var(--ink-2)",
               font: "inherit", fontSize: "var(--fs-xs)", padding: "0 6px",
               cursor: "pointer" },
      text: "claims: " + claims.length + " ›",
      onclick: function () {
        pickClaim(MapClaim.index === null ? 0
                  : (MapClaim.index + 1) % claims.length);
      },
    }) : null;

    // the legend: the four marks that matter on one line; the rest, and
    // the keys, behind a disclosure
    var foot = H("div", { class: "tjm-foot" }, [
      legendKey("solid line", "matched"),
      legendKey("dashed line", "drifted / divergent, with the aligner's score"),
    ...
    var more = H("details", { class: "tjm-legend-more" }, [
      H("summary", { text: "full legend and keys" }),
      H("div", { class: "tjm-foot" }, [
  ...
  function ensureLensStyle() {
    ...
    var css = [
      ...
      ".tjl-idx{font-family:var(--mono);font-size:var(--fs-xs);color:var(--ink-3);flex:none;",
      "width:20px;text-align:right}",
      ".tjl-name{font-family:var(--mono);font-size:var(--fs-xs);color:var(--ink);min-width:0;",
      "overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
      ".tjl-badges{margin-left:auto;display:inline-flex;gap:4px;flex:none;align-items:center}",
      ".tjl-b{font-size:var(--fs-xs);text-transform:uppercase;letter-spacing:.06em;",
  ...
  AgentDiff.block({
    ...
    relevance: function (ctx) {
      ...
      if (!report) return 0;
      if (!stepsOf(report, "a").length && !stepsOf(report, "b").length) return 0;
      // in the story the reading carries the lens as its "every step" fold
      try { if (document.body.getAttribute("data-view") === "story") return 0; } catch (err) { /* fine */ }
      return 0.68;
    },
    render: function (el, ctx) {
      bind(ctx);
  ...
  /* The lens drawn into any host — the reading's "every step" fold uses
   * it, following the reading's own A/B choice. */
  AgentDiff.renderLens = function (host, ctx, side) {
    bind(ctx);
    ensureStyle();
    ensureLensStyle();
    lensSync(ctx.report);
    if (side === "a" || side === "b") Lens.side = side;
    function paint() { host.innerHTML = ""; drawLens(host, ctx); }
    paint();
    subscribe(host, paint);
  };
