...
(function (global) {
  ...
  function color(side) { return L.color.side(side, "rl"); }
  //: a reward with its sign: +12.4, −1.0, +0.05, 0 — adaptive precision, unlike L.fmt.signed
  function reward(v) {
    if (!isNum(v)) return "—";
    if (v === 0) return "0";
    var a = Math.abs(v);
  ...
  function leafText(r) { return "step " + r.step + (r.name ? " · " + r.name : "") + " · " + reward(r.reward) + (labelsOf(r) ? " · " + labelsOf(r) : ""); }
  function leafTip(report, side, r) {
    return [{ b: true, text: name(report, side) + " · step " + r.step + (r.name ? " · " + r.name : "") + (r.agent ? " · " + r.agent : "") },
      { text: "reward " + reward(r.reward) + " · return so far " + reward(r.cum) + (isNum(r.to_go) ? " · to go " + reward(r.to_go) : "") + (isNum(r.credit) ? " · credit " + reward(r.credit) : "") },
  ...
  function drawCurve(host, report, rl, tip, W, sides) {
    ...
    var svg = d3.select(host).append("svg").attr("class", "rl-curve").attr("viewBox", "0 0 " + W + " " + H).attr("role", "img")
      .attr("aria-label", "cumulative return over steps, " + sides.map(function (s) { return name(report, s) + " " + reward(rl[s]["return"]); }).join(" and ") +
    ...
    var yd = y.domain();
    if (yd[1] > 0) svg.append("text").attr("class", "rl-ax").attr("x", m.l - 6).attr("y", y(yd[1]) + 4).attr("text-anchor", "end").text(reward(yd[1]));
    if (yd[0] < 0) svg.append("text").attr("class", "rl-ax").attr("x", m.l - 6).attr("y", y(yd[0]) + 4).attr("text-anchor", "end").text(reward(yd[0]));
    ...
    sides.forEach(function (s) {
      ...
      svg.append("path").attr("class", "rl-cum").attr("data-side", s).attr("d", line(pts)).attr("fill", "none")
        ...
        .append("title").text(name(report, s) + " · return " + reward(rl[s]["return"]) + " · discounted " + reward(rl[s].discounted_return));
      ...
      if (isNum(ans)) {
        svg.append("rect").attr("class", "rl-answer").attr("data-side", s).attr("x", x(ans) - 3).attr("y", y(cumAt(rs[s], ans)) - 3).attr("width", 6).attr("height", 6).attr("fill", color(s))
          .append("title").text(name(report, s) + " · the answer at step " + ans + " · return " + reward(cumAt(rs[s], ans)));
      }
    ...
    if (!narrow) {
      ...
      ends.forEach(function (e) {
        svg.append("text").attr("class", "rl-end").attr("data-side", e.s).attr("x", W - m.r + 6).attr("y", e.y + 4).attr("fill", color(e.s)).attr("font-weight", 600).text(reward(e.v));
      });
    }
    ...
    if (parted !== null) {
      ...
      svg.append("circle").attr("class", "rl-parted").attr("data-step", parted).attr("cx", px).attr("cy", (ya + yb) / 2).attr("r", 5).attr("fill", "none").attr("stroke", "var(--ink)").attr("stroke-width", 1.4)
        .append("title").text("the returns part at step " + parted + ": " + name(report, "a") + " " + reward(cumAt(rs.a, parted)) + ", " + name(report, "b") + " " + reward(cumAt(rs.b, parted)));
  ...
  function drawBands(host, report, rl, st, tip, ctx, W, sides, repaint) {
    ...
    function runLabel(s) { return narrow ? name(report, s) + " · " + reward(rl[s]["return"]) : name(report, s) + " · " + (isNum(rl[s].steps) ? rl[s].steps + " steps · " : "") + "return " + reward(rl[s]["return"]); }
    var x0 = 0;
    ...
    ["a", "b"].forEach(function (side) {
      ...
      var svg = d3.select(host).append("svg").attr("class", "rl-band").attr("data-side", side)
        ...
        .attr("aria-label", name(report, side) + " as a reward thread: " + cs.length + " stretches, " + hot + " hot; " + rs.length + " rewarded steps, return " + reward(run["return"]) + ", " + ticks.length + " at or above the 90th percentile; length is reward impact");
      ...
      rg.on("pointermove", function (evt) { tip.show(evt, [{ b: true, text: name(report, side) + " · return " + reward(run["return"]) + " · discounted " + reward(run.discounted_return) }, { text: (run.positive || 0) + " gains · " + (run.negative || 0) + " losses · " + (run.zero || 0) + " zero" }, run.narrative ? { text: run.narrative } : null]); }).on("pointerleave", tip.hide);
      ...
      p.units.forEach(function (u, ui) {
        ...
        if (u.type === "fold") {
          ...
          if (isNum(fcredit) && creditMax > 0) {
            svg.append("rect").attr("class", "rl-credit").attr("data-id", u.id).attr("data-fold", "1").attr("data-side", side).attr("data-credit", fcredit.toFixed(4))
              ...
              .append("title").text(name(report, side) + " · " + u.clusters.length + " quiet stretches folded · credit " + reward(fcredit));
          }
          fg.append("title").text(u.steps + " steps · " + secs(u.seconds) + " · reward " + reward(sumR) + " — " + u.clusters.length + " quiet stretches folded; click to open");
          fg.on("pointermove", function (evt) {
            tip.show(evt, [{ b: true, text: name(report, side) + " · ×" + u.steps + " steps folded" }, { text: u.steps + " steps · " + secs(u.seconds) + " · reward " + reward(sumR) + (u.lanes.length ? " · " + u.lanes.join(", ") : "") }, { text: "steps " + u.from + "–" + u.to + " · " + u.clusters.length + " quiet stretches · click to open" }]);
        ...
        cg.append("title").text(name(report, side) + " · " + (c.label || c.kind) + " · steps " + c.from + "–" + c.to + " · " + secs(c.seconds) + " · reward " + reward(sum) + (isNum(credit) ? " · credit " + reward(credit) : "") + " · impact " + imp.toFixed(2) + (c.why ? " — " + c.why : ""));
        cg.on("pointermove", function (evt) {
          tip.show(evt, [{ b: true, text: name(report, side) + (c.lane ? " · " + c.lane : "") + " · " + (c.label || c.kind) },
            ...
            { text: "reward " + reward(sum) + " over " + n + " rewarded step" + (n === 1 ? "" : "s") + (isNum(credit) ? " · credit " + reward(credit) : "") },
        ...
        if (isNum(credit) && creditMax > 0) {
          ...
          svg.append("rect").attr("class", "rl-credit").attr("data-id", c.id).attr("data-side", side).attr("data-credit", credit.toFixed(4))
            ...
            .append("title").text(name(report, side) + " · " + (c.label || c.kind) + " · credit " + reward(credit) + " (" + ((run.credit && run.credit.source) || "credit") + ")");
        }
      });
      ...
      if (isNum(ans)) {
        svg.append("rect").attr("class", "rl-answer").attr("data-step", ans).attr("x", stepX(ans) - 3.5).attr("y", yT - 3.5).attr("width", 7).attr("height", 7).attr("fill", color(side))
          .append("title").text("the answer at step " + ans + " · return " + reward(cumAt(rs, ans)));
      }
      ...
      openUnits.forEach(function (u) {
        ...
        var why = svg.append("text").attr("class", "rl-why").attr("data-id", c.id).attr("x", lx - 4).attr("y", y0 + 4)
          .text(fit((c.label ? c.label + " · " : "") + "steps " + c.from + "–" + c.to + " · " + secs(c.seconds) + " · reward " + reward(sum) + (isNum(credit) ? " · credit " + reward(credit) : "") + (c.why ? " — " + c.why : ""), W - padR - lx + 4));
  ...
  function prefLine(H, report, rl) {
    ...
    var tail = (basis ? " — basis: " + basis : "") + (isNum(pref.margin) ? " · margin " + reward(pref.margin) : "");
    ...
    render: function (el, ctx) {
      ...
      ["a", "b"].forEach(function (s) {
        ...
        var chip = H("span", { class: "rl-chip rl-run", "data-side": s, title: run.narrative || "" }, [
          ...
          document.createTextNode(" · return " + reward(run["return"]) + " · discounted " + reward(run.discounted_return) + " · "),
      ...
      ["a", "b"].forEach(function (side) {
        ...
        (Array.isArray(run.largest) ? run.largest : []).forEach(function (l) {
          ...
          t.appendChild(H("tr", { "data-side": side, "data-step": l.step }, [
            ...
            H("td", { class: "n " + (v > 0 ? "pos" : v < 0 ? "neg" : ""), text: reward(v) }),
            H("td", { text: l.why || labelsOf(r) || "" }),
            H("td", { class: "n", text: isNum(r.credit) ? reward(r.credit) : "—" }),
          ]));
        });
      });
      ...
      ["a", "b"].forEach(function (side) {
        ...
        clustersOf(run).forEach(function (c) {
          ...
          t2.appendChild(H("tr", { "data-side": side, "data-id": c.id }, [
            ...
            H("td", { class: "n " + (sum > 0 ? "pos" : sum < 0 ? "neg" : ""), text: reward(sum) }),
            H("td", { class: "n", text: isNum(credit) ? reward(credit) : "—" }),
