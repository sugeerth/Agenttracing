...
(function (global) {
  ...
  function ensureStyle() {
    L.style.once("scorecard", [
      ...
      ".sc-detect td.bad{color:var(--bad);font-weight:600}.sc-detect td.warn{color:var(--warn)}.sc-detect code{font-family:var(--mono);font-size:var(--fs-xs)}",
    ].join(""));
  }
  ...
  // The card scored against runs whose verdict is already known. It is the
  // only section here that measures the *measurement*, so it says plainly
  // what nothing caught rather than leaving the reader to notice the gap.
  function detectionSection(H, card) {
    var det = card.detection;
    if (!det) return null;
    var sec = H("section", { class: "sc-sec", "data-sec": "detection" });
    sec.appendChild(H("h4", { text: "Does the evaluation see it?" }));
    if (!det.measurable) {
      sec.appendChild(H("p", { class: "sc-note", "data-role": "why", text: det.reason }));
      return sec;
    }
    sec.appendChild(H("p", { class: "sub", "data-role": "narrative", text: det.narrative }));
    var t = H("table", { class: "sc-table sc-detect" });
    t.appendChild(H("tr", null, ["known failure", "run", "graded", "caught by"].map(function (h) { return H("th", { text: h }); })));
    det.modes.forEach(function (row) {
      var caught = row.signals && row.signals.length;
      t.appendChild(H("tr", { "data-mode": row.mode, "data-caught": caught ? "true" : "false" }, [
        H("td", null, [H("code", { text: row.mode })]),
        H("td", { class: "dim", text: row.task + " · " + row.agent }),
        H("td", { class: row.graded_pass ? "warn" : "", text: row.graded_pass ? "pass" : "fail" }),
        H("td", { class: caught ? "" : "bad" }, caught ? [H("span", { text: row.signals.join(", ") })] : [H("b", { text: "nothing" })]),
      ]));
    });
    sec.appendChild(H("div", { class: "scroll-x" }, [t]));
    var fp = (det.controls || {}).false_positives || [];
    sec.appendChild(H("p", { class: "sc-note", "data-role": "controls",
      text: fp.length
  ...
  AgentDiff.block({
    ...
    render: function (el, ctx) {
      ...
      var detect = detectionSection(H, card);
      if (detect) root.appendChild(detect);
